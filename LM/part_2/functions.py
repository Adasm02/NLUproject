...
def create_language_model(embedding_dim, hidden_dim, lr, lr_decay, device, emb_dropout, output_dropout, 
                          lang, opt_type, weight_tying, drop_out_mode):
    """
    Creates and initializes the language model.
    """
    vocab_size = len(lang.word2id)
    model = LSTMLanguageModel(embedding_dim, hidden_dim, vocab_size, out_dropout=output_dropout, emb_dropout=emb_dropout, 
                              weight_tying=weight_tying, drop_out_mode=drop_out_mode).to(device)
    model.apply(init_weights)
    
    optimizer = optim.SGD(model.parameters(), lr=lr) if opt_type == 'SGD' else optim.AdamW(model.parameters(), lr=lr) 
